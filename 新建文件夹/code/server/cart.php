...
if($type == "add")
{
  ...
  $sql = "SELECT * FROM yiyao WHERE good_id = $good_id";
  $result = mysqli_query($db,$sql);
  if(mysqli_num_rows($result) == 0)
  {
    /* 往数据库表中新增一条数据 */
    $sql = "INSERT INTO `yiyao` (`good_id`, `src`, `title`,`num`) VALUES ($good_id, '$src', '$title',$num)";
  }else{
    /* 更新数据 */
  ...
  $sql = "SELECT yiyao.*,goods.src,goods.shopName,goods.price FROM  yiyao, goods WHERE yiyao.good_id = goods.good_id";
...
{
  $good_id = $_REQUEST["good_id"];
  $delSql = "DELETE FROM `yiyao` WHERE good_id = $good_id";
